...
1. 成员进入项目的“任务规划”，填写目标并点击“生成任务草案”。生成成功后自动保存，可编辑后保存新版本。
2. 在预览区点击“提交这一版”，直接使用已保存草案送审，不再次调用模型。完成后显示“等待审批”；刷新页面后在“审批记录”中仍可查看记录、任务、假设、风险和提交时的文档依据。草案版本与事务细节见[实现方案](transaction-and-drafts-plan.md)。
