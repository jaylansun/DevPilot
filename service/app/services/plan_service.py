...
def validate_proposal_sources(
    proposal: PlanProposalVO, reader: PlanningReadService
) -> None:
    ...
    if not {"search_documents", "read_task_board"} <= executed:
        raise ApiError(
            502, "planning_missing_tools", "模型尚未完成文档和看板读取，请重试"
        )
    ...
    if not selected or not selected <= available:
        raise ApiError(
            502, "invalid_plan_sources", "任务方案没有提供有效的文档依据，请重试"
        )
...
class PlanService:
    ...
    def __init__(
        self, settings: Settings, index_service, session_factory, agent_service
    ):
        ...
        *,
        ...
        if not await list_ready_documents(session, project_id):
            raise ApiError(
                ...
                "请先上传项目文档并等待索引完成，再生成任务草案",
            )
        if not self.configured:
            raise ApiError(
                503,
                "model_not_configured",
                "真实规划尚未配置模型，请设置 MODEL_NAME、LLM_API_KEY 和 LLM_BASE_URL",
            )
        try:
        ...
        try:
            ...
            raise ApiError(
                504, "planning_timeout", "任务规划超时，请缩小目标范围后重试"
            ) from exc
        except (
