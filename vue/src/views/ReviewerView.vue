...
<template>
  <section aria-label="审批中心" class="min-w-0">
    <div class="ui-page-heading"><div><p class="ui-eyebrow">让每一份计划更可靠</p><h1>审批工作区</h1><p class="ui-muted">核对任务、风险与依据，批准后加入项目看板。</p></div></div>
    ...
    <div class="grid min-w-0 items-start gap-5 desktop:grid-cols-[280px_minmax(0,1fr)]">
      <section aria-label="审批列表" class="min-w-0 space-y-3">
        <p v-if="!items.length && !loading" class="rounded-2xl bg-surface p-6 text-sm leading-7 text-muted">当前没有符合条件的审批方案。</p>
        <button v-for="item in items" :key="item.id" type="button" :disabled="busy" :aria-pressed="selected?.id === item.id" class="ui-interactive block w-full cursor-pointer rounded-2xl border border-line bg-surface p-4 text-left disabled:cursor-wait focus-visible:outline-brand" :class="{ 'border-brand!': selected?.id === item.id }" @click="choose(item)">
          <span class="block text-sm font-semibold wrap-anywhere">{{ item.project_name }}</span><span class="mt-2 block text-sm leading-7 text-muted wrap-anywhere">{{ item.goal }}</span><span class="mt-3 block text-xs text-brand">{{ labels[item.status] }}</span>
        </button>
      ...
      <section v-if="selected" class="min-w-0 rounded-2xl bg-surface p-5 mobile:p-7" aria-label="审阅方案">
        ...
        <div v-if="selected.status === 'pending'" class="mt-6 border-t border-line pt-5">
          ...
          <div class="mt-5 flex flex-wrap gap-3 [&>.el-button+.el-button]:ml-0">
        ...
        <div v-if="selected.status === 'processing'" class="mt-5">
          <p class="text-sm leading-7 text-muted">审批决定已保存。若执行曾中断，原审批人可继续执行；重复执行不会重复创建任务。</p>
